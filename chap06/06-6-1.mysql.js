...
// 데이터베이스에 연결하면 메모리 리소스를 많이 차지하므로 한번 만든 연결 객체는
// 커넥션 풀에 넣어두고 다음번 요청이 있을 때 재사용함.
...
var authUser = function(id, password, callback) {
    ...
    pool.getConnection(function(err, conn) {
        ...
        // SQL 문 실행
        // ?를 모두 하나로만 하면 아래처럼 나옴
        // select 'id', 'name', 'age' from 'users' where id = 'juhyuun' and password = '1111'
        ...
        conn.on('error', function(err) {
            console.log('데이터베이스 연결 시 에러 발생 : ', err);
            callback(err, null);
        });
    });
};
